# Conset PDF Test Corpus

This directory contains the torture corpus used to validate the Conset PDF extraction engine across different complexity levels and edge cases.

## Torture Corpus Tier System

The corpus is organized into progressive difficulty tiers, each designed to validate specific capabilities and robustness of the extraction engine:
...
#[test]
fn test_tier1_corpus() {
    let corpus_dir = Path::new("tests/corpus/tier1");
...
```bash
# Test baseline corpus only
cargo test tier1 -- --nocapture

# Test all tiers
cargo test corpus -- --nocapture

# Test with specific corpus file
cargo test -- --exact test_corpus_example --nocapture
```
...
Corpus files that become redundant or outdated should be:
