...
| G-006 | audit | `crates/audit/src/` | Audit hook integration | MISSING-WIRING | MEDIUM | `ARCHITECTURE.md` — Audit events generated during extraction and processing | **Closed** — Phase 4 Task 4.8 confirmed all 7 original CLI handlers (`extract`, `visualize`, `segment`, `visualize-segments`, `parse`, `visualize-ast`, `edit`) emit `AuditEventData::OperationStarted` and `AuditEventData::OperationEnded` via `AuditBundle`. Phase 6 added the 8th handler (`stitch`), wired identically. All 9 handlers now covered. Evidence: `apps/backend-cli/src/handlers/edit.rs`, `apps/backend-cli/src/handlers/stitch.rs`, and all sibling handlers in `apps/backend-cli/src/handlers/`. |
...
| G-013 | contracts + workflows | (intake boundary) | Multi-file intake bundle contract: `IntakeBundle` type, multi-input support in `WorkflowRequest`, `IntakeIssue` artifact type | MISSING-WIRING | HIGH | `MASTER_PLAN.md` Non-Negotiable #22 — Intake Triage is mandatory pre-Lexer stage | **Closed** — Sprint 8.2 implemented `IntakeBundle`, `IntakeFile`, `IntakeRole`, `IssueSeverity`, `IntakeIssue`, `TriageConfidence`, `DocumentClass` types in `crates/contracts/src/intake.rs`; `WorkflowOperation::Intake` added to contracts; `intake_bundle: Option<IntakeBundle>` added to `WorkflowRequest` (`#[serde(default)]`); `NormalizedIntakeBundle` type defined. Evidence: `crates/contracts/src/intake.rs`, `apps/backend-cli/src/handlers/intake.rs`, `apps/backend-cli/tests/cli_integration_test.rs` (3 Phase 8.2 CLI integration tests pass). |
...
| G-016 | engine / workflows | (intake boundary) | Page rotation detection and normalization: PDFium read of page-dict metadata, `lopdf` write for `/Rotate` correction, per-page normalization manifest | MISSING-WIRING | MEDIUM | `MASTER_PLAN.md` Non-Negotiable #23 — `lopdf` is canonical PDF write backend for page-level ops including rotation normalization | **Closed** — Sprint 8.2 implemented `Stage0Normalizer` in `crates/engine/src/intake.rs`: PDFium `/Rotate` page-dict metadata read; `lopdf` in-place rotation normalization for pages with non-zero rotation; per-page normalization manifest output (`NormalizedIntakeBundle`); `intake` CLI subcommand with `--input`, `--output`, `--dry-run`. Evidence: `crates/engine/src/intake.rs`, `apps/backend-cli/src/handlers/intake.rs`, `apps/backend-cli/tests/cli_integration_test.rs` (3 Phase 8.2 CLI integration tests pass). |
...
| G-018 | engine / workflows | (drawing detection boundary) | Deterministic drawing-frame and corner-band detector for title-block candidate generation (vector geometry pass). **Phase 0.5 note:** `detect_title_block_anchor()` in `tools/pattern_dev.rs` implements keyword-label density corner scoring (Phase G bonus); `TitleBlockSidecar` schema pre-seeds 4 corner candidates (BR/BL/TR/TL). Full shared-engine frame/corner detector remains Open for Band 1. | MISSING-WIRING | HIGH | `MASTER_PLAN.md` + `ARCHITECTURE.md` — vector-first deterministic title-block localization is baseline | **Partially addressed (Phase 9.1)** — `DrawingSegmentEngine::build_index()` in `crates/engine/src/drawing_segment.rs` implements keyword-anchor span-density scanning of the bottom page band (~Y > 0.85), providing working sheet boundary detection for RWB LHHS fixtures. Full vector-geometry frame/corner-band detector still open. |
...
| G-020 | engine / workflows | (field extraction boundary) | Deterministic sheet number/title extraction inside title-block ROI using pattern + typography + relative-position scoring | MISSING-WIRING | HIGH | `MASTER_PLAN.md` + `ARCHITECTURE.md` — sheet fields must be extracted from ROI with auditable scoring evidence | **Partially closed (Phase 9.1)** — `DrawingSegmentEngine` extracts `sheet_id` and `sheet_title` from span clusters within the detected title-block band; reliably identifies `A-101 / FIRST FLOOR PLAN` style entries on RWB LHHS fixtures. Full auditable geometry scoring with pattern + typography + relative-position tie-break evidence still open. |
...
| G-028 | engine / workflows | (table boundary) | Schedule/table parser to canonical schema with page/region provenance and confidence per extracted record | MISSING-WIRING | HIGH | `MASTER_PLAN.md` Non-Negotiable #30 — schedule parsing must emit schema-versioned structured outputs | **Closed (Sprint 10.4)** — `build_equipment_dataset` in `crates/engine/src/submittal_export.rs` assembles `TidyRow` records with `page`, `bbox`, `confidence`, and `source` fields; `extract-submittal` CLI emits `EquipmentDataset` JSON/CSV with `schema_version = "1.0.0"`. Drawing schedule tables addressed via `extract-schedules` (Sprint 9.4). G-028 fully closed. |
| G-029 | workflows / contracts | (export boundary) | Schema-versioned schedule export adapters for JSON/CSV/CSV with stable field mapping contract | MISSING-WIRING | MEDIUM | `MASTER_PLAN.md` Non-Negotiable #30 — machine-consumable export formats JSON/CSV/XML required | **Partially Closed (Sprint 10.4)** — `dataset_to_json` (pretty-printed `EquipmentDataset`) and `dataset_to_csv` (14-column RFC 4180 CSV) implemented for submittal data; drawing schedule export (`extract-schedules --format json`) implemented Sprint 9.4. XML export not yet implemented. |
...
| 2.5.0 | 2026-04-08 | Closed G-013 and G-016 with Sprint 8.2 evidence (`IntakeBundle` contract types + `Stage0Normalizer` rotation normalization both shipped in Sprint 8.2, April 8, 2026). |
