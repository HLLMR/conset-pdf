...
|---|---|---|---|---|
| `crates/ir` data model and validation | **Implemented** — `LayoutTranscript`, `Page`, `Span`, `BBox`, `SegmentIndex`, `SectionEntry`, `ParsedDocument`, `SectionAst`, `AstNode`, `OutlineTag` all implemented and tested; `validate_transcript()` enforced in engine pipeline; `Validator::validate()` delegates to `validate_transcript()` (G-007 closed); `Document` and `Element` stubs remain for future use | Complete Document/Element types | `crates/ir/src/`, `crates/ir/tests/` | G-008 (Document/Element stubs) |
| `crates/pdf-extraction` extraction baseline | **Implemented** — `PdfiumExtractor::extract_page()` returns real `PageData` with `Vec<SpanData>` (text, font name, font size, `RawBBox`), `width_pts`, `height_pts`; `load_document`, `get_page_count`, `extract_text` all implemented and tested; PDF→IR span conversion wired in engine extraction pipeline (G-005 closed) | — | `crates/pdf-extraction/src/extractor.rs`, `crates/pdf-extraction/src/types.rs` | — |
| `crates/engine` processing baseline | **Implemented through Phase 4** — extraction wired (`pipeline/extraction.rs`), validation wired (`pipeline/parsing.rs`, G-002 closed), segmentation engine (`segment.rs`): cluster-based footer CSI section ID detection, FOOTER_Y/FOOTER_CLUSTER_GAP constants, `detect_section_id()`, `inject_missing_parts()` recovery; parse engine (`parse.rs`): 5-level CSI outline tree (Part→Article→Paragraph→SubParagraph→SubSubParagraph→SubSubSubParagraph), y-proximity line clustering, x-sort, continuation folding, noise-skip; HTML visualizer (`visualize_ast.rs`); edit engine (`edit.rs`): `SectionEditor` with pre-flight validation, `apply_delete_to_section`, `apply_replace_to_section`, `apply_insert_after_to_section`, CSI-canonical `renumber_siblings`; `backend-cli` handles: extract, visualize, segment, visualize-segments, parse, visualize-ast, edit; G-001, G-002, G-003 (accepted), G-005, G-006, G-007, G-009, G-022 all closed; 26/26 integration tests pass; main.rs prints version only (deliberate — G-003 accepted) | Phase 5+ rendering, stitching | `crates/engine/src/`, `apps/backend-cli/tests/cli_integration_test.rs` | G-008 (Document/Element stubs) |
| `crates/audit` bundle/event writer baseline | **Integrated** — AuditEvent, AuditBundle, event types, JSON writer/reader all present; all 7 CLI handlers now emit `OperationStarted`/`OperationEnded` events (G-006 closed in Phase 4) | Stronger event ordering/persistence tests | ../crates/audit/src/ | G-010 (trivial tests) |
...
|---|---|---|---|---|
| Deterministic extraction + edit pipeline | **Implemented through Phase 4** — real PDFium extraction wired; `SpanData`→`Span` conversion with normalized coords (top-left, [0,1]); `validate_transcript()` called in pipeline; extract + segment + parse + edit pipeline operational end-to-end; all handlers emit `OperationStarted`/`OperationEnded` audit events | Phase 5+ render/stitch | `crates/engine/src/pipeline/extraction.rs`, `crates/engine/src/edit.rs`, `apps/backend-cli/tests/cli_integration_test.rs` | G-003 (accepted) |
