...
| `crates/engine` processing baseline | **Implemented through Phase 3** — extraction wired (`pipeline/extraction.rs`), validation wired (`pipeline/parsing.rs`, G-002 closed), segmentation engine (`segment.rs`): cluster-based footer CSI section ID detection, FOOTER_Y/FOOTER_CLUSTER_GAP constants, `detect_section_id()`, `inject_missing_parts()` recovery; parse engine (`parse.rs`): 5-level CSI outline tree (Part→Article→Paragraph→SubParagraph→SubSubParagraph→SubSubSubParagraph), y-proximity line clustering, x-sort, continuation folding, noise-skip; HTML visualizer (`visualize_ast.rs`); `backend-cli` handles: extract, visualize, segment, visualize-segments, parse, visualize-ast; G-001, G-002, G-005, G-007, G-009, G-022 all closed; 19/19 integration tests pass; main.rs still prints version only (G-003 open, coverage via backend-cli) | Phase 4+ edit operations, rendering, stitching | `crates/engine/src/`, `apps/backend-cli/tests/cli_integration_test.rs` | G-003 (main.rs CLI), G-006 (audit hooks) |
...
|---|---|---|---|---|
| Deterministic extraction pipeline | **Implemented** — real PDFium extraction wired; `SpanData`→`Span` conversion with normalized coords (top-left, [0,1]); `validate_transcript()` called in pipeline; extract + segment + parse pipeline operational end-to-end | Phase 4+ edit/render/stitch | `crates/engine/src/pipeline/extraction.rs`, `apps/backend-cli/tests/cli_integration_test.rs` | G-003 (engine main.rs), G-006 (audit hooks) |
