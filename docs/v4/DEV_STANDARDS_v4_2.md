...
3. Do not compare floating-point geometry values directly for equality in correctness-sensitive logic.
4. Hash contracts must exclude timestamps and include deterministic content fields only.
5. Fallback paths are allowed only when they preserve deterministic guarantees; otherwise fail explicitly.

### Session and Operation Logging

1. Log operation start and end explicitly, including duration.
2. Include context metadata in timing logs (at least page count, file size bucket, and detection source).
3. Ensure crash and unhandled-error logs are flushed before process exit.
4. For deprecation gates, document whether behavior is hard-fail or soft-warn; do not mix semantics implicitly.

### Regression Priority Areas

Bookmark destination mapping is regression-sensitive and must maintain dedicated regression coverage for refactors.
