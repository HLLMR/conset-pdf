...
fn test_normalize_bbox_converts_origin() {
    ...
    let normalized = conset_pdf_ir::normalize_bbox(&bbox, &page)
        .expect("normalize_bbox should succeed");

    assert!(approx_eq(normalized.y, 1.0 - (50.0 / 792.0)),
        "y_norm should be near 0.937, got {}", normalized.y);
}

#[test]
fn test_normalize_bbox_flips_y_axis() {
    // Page: 100x100 (simple math)
    let page = Page::new(0, 100.0, 100.0)
        .expect("valid page dimensions");
    ...
    assert!(normalized.y > 0.5,
        "y_norm should be in upper portion (> 0.5), got {}", normalized.y);
    assert!(approx_eq(normalized.y, 0.9),
        "y_norm should be near 0.9, got {}", normalized.y);
}
...
fn test_normalize_bbox_scales_width() {
    ...
    assert!(approx_eq(normalized.width, 0.5),
        "width_norm should be 0.5, got {}", normalized.width);
}
...
fn test_normalize_bbox_handles_full_page() {
    ...
    assert!(approx_eq(normalized.x, 0.0),
        "x should be 0.0 for full page, got {}", normalized.x);
    assert!(approx_eq(normalized.y, 0.0),
        "y should be 0.0 for full page, got {}", normalized.y);
    assert!(approx_eq(normalized.width, 1.0),
        "width should be 1.0 for full page, got {}", normalized.width);
    assert!(approx_eq(normalized.height, 1.0),
        "height should be 1.0 for full page, got {}", normalized.height);
}
...
fn test_normalize_bbox_rejects_oversized_coords() {
    ...
    assert!(result.is_err(),
        "normalize_bbox should return Err for oversized coordinates");
}
...
fn test_normalize_bbox_handles_fractional_coords() {
    ...
    assert!(approx_eq(normalized.x, 33.33 / 100.0),
        "x_norm should be ~0.3333, got {}", normalized.x);
    assert!(approx_eq(normalized.width, 10.5 / 100.0),
        "width_norm should be ~0.105, got {}", normalized.width);
    assert!(approx_eq(normalized.height, 5.25 / 100.0),
        "height_norm should be ~0.0525, got {}", normalized.height);
}
