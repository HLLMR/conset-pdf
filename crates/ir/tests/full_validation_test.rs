...
use conset_pdf_ir::{validation, BBox, LayoutTranscript, Page, Span, TranscriptMetadata};
...
fn test_validate_transcript_rejects_unsorted_spans() {
    ...
    let result = validation::validate_transcript(&transcript);
    assert!(result.is_ok(), "Transcript with properly sorted spans should pass validation");
}
...
fn test_validate_transcript_rejects_noncontiguous_pages() {
    ...
    assert!(result.is_err(), "Transcript with non-contiguous pages should fail validation");
}
...
fn test_validate_transcript_provides_actionable_errors() {
    ...
    for i in 0..5 {
        let span = Span::new(
            &format!("span{}", i),
            BBox::new(0.1 + (i as f64 * 0.1), 0.1 + (i as f64 * 0.05), 0.05, 0.05).unwrap(),
            12.0,
        )
        .unwrap();
    ...
    for i in 0..3 {
        let span = Span::new(
            &format!("page1_span{}", i),
            BBox::new(0.1 + (i as f64 * 0.1), 0.1, 0.05, 0.05).unwrap(),
            12.0,
        )
        .unwrap();
    ...
    if let Err(err) = result {
        ...
        assert!(!error_msg.is_empty(), "Error message should be actionable and not empty");
    }
}
